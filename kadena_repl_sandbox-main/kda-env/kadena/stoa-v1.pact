...
(module STOA GOV
    ...
    ;;GOVERNANCE
    ;;{G1}
    (defconst STOA|MasterOne                "namespace.stoa_master_one")
    (defconst STOA|MasterTwo                "namespace.stoa_master_two")
    (defconst STOA|MasterThree              "namespace.stoa_master_three")
    (defconst STOA|MasterFour               "namespace.stoa_master_four")
    (defconst STOA|MasterFive               "namespace.stoa_master_five")
    (defconst STOA|MasterSix                "namespace.stoa_master_six")
    (defconst STOA|MasterSeven              "namespace.stoa_master_seven")
    ;;{G2}
    ...
    (defun C_RotateAccount (account:string new-guard:guard)
    ...
    ;;{F7}  [X]
    (defun X_Coinbase:string (account:string account:guard amount:decimal)
        @doc "Internal function for the initial creation of coins. \
        \ This function cannot be used outside of the coin contract"
        (UEV_Account account)
        (UEV_StoaPrecision amount)
        (require-capability (COINBASE))
        (emit-event (TRANSFER "" account amount))
        (with-capability (CREDIT account)
            (X_Credit account account-guard amount)
        )
    )
    ...
    ;;
    (defpact X_FundTX (sender:string miner:string miner-guard:guard total:decimal)  
        ...
        (step (X_BuyGas sender total))
        (step (X_RedeemGas miner miner-guard sender total))
    )
    (defun X_BuyGas:string (sender:string total:decimal)
    ...
    (defun X_RedeemGas:string (miner:string miner-guard:guard sender:string total:decimal)
        ...
        (let
            ...
            (emit-event (TRASNFER sender miner fee))
            (with-capability (CREDIT sender)
                ;; Update Directly instead of credit
                (if (> refun 0.0)
                    (update StoaTable account
                        {"balance" : (+ (UR_Balance account) refund)}
                    )
                    "noop"
                )
            )
    ...
    (defun X_Remediate (account:string amount:string)
        ...
        (UEV_Account account)
        (UEV_Amount total "Remediation amount must be positive")
        (UEV_StoaPrecision amount)
