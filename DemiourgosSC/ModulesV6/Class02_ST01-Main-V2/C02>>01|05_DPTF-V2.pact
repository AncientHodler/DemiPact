...
(module DPTF GOV
    ...
    (defun UR_Hibernation:string (id:string)
        (let 
            (
                ...
                (sentinel:string "__MISSING_<hibernation-link>__")
                (temp:string (try sentinel (at "hibernation-link" (read DPTF|PropertiesTable id ["hibernation-link"]))))
                (needs-populate:bool (= temp sentinel))
                (link:string (if needs-populate default-value temp))
            )
            (if needs-populate
    ...
    (defun UR_DirectRecovery:bool (id:string)
        (let 
            (
                (default-value:bool false)
                (sentinel:string "__MISSING_<direct-recovery>__")
                (temp (try sentinel (at "direct-recovery" (read DPTF|PropertiesTable id ["direct-recovery"]))))
    ...
    (defun C_DeployAccount (id:string account:string)
        ...
        (let
            ...
            (with-default-read DPTF|BalanceTable (UC_IdAccount id account)
                ...
                (write DPTF|BalanceTable (UC_IdAccount id account)
    ...
    (defun C_Burn:object{IgnisCollectorV2.OutputCumulator}
        ...
        (let
            ...
            (with-capability (DPTF|C>BURN id account amount)
                (XB_DebitTrueFungible account id amount (ref-U|DPTF::EmptyDispo) false)
    ...
    (defun C_Mint:object{IgnisCollectorV2.OutputCumulator}
        ...
        (let
            ...
            (with-capability (DPTF|C>MINT id account amount origin)
                (XB_CreditTrueFungible account id amount)
                (XB_UpdateSupply id amount false)
                (if origin
                    (update DPTF|PropertiesTable id
                        {"origin-mint"          : false
    ...
    (defun C_WipeSlim:object{IgnisCollectorV2.OutputCumulator}
        ...
        (let
            ...
            (with-capability (DPTF|C>WIPE-SLIM id account-to-be-wiped amount-to-be-wiped)
                (XB_DebitTrueFungible account-to-be-wiped id amount-to-be-wiped (ref-U|DPTF::EmptyDispo) true)
    ...
    ;;
    ;;
    (defun XB_DebitTrueFungible (account:string id:string amount:decimal dispo-data:object{UtilityDptf.DispoData} wipe-mode:bool)
    ...
    (defun XB_CreditTrueFungible (account:string id:string amount:decimal)
        @doc "Debit DPTF <id> on <account> with <amount> \
            \ Only Performs Debitation, does not update supply"
        (UEV_IMC)
