...
(module DALOS GOV
    ...
    (defun UR_TrueFungible:object{OuronetDalosV5.DPTF|BalanceSchemaV2} 
    ...
    (defun UEV_NotSmartOuronetAccount (account:string)
        (enforce (not (UR_AutonomicRoles)) "Non Smart Ouronet Accounts required for exec")
    )
    ...
    (defun XB_UpdateBalance (account:string snake-or-gas:bool new-balance:decimal)
        ...
        (with-capability (SECURE)
            (XI_UpdateTF account snake-or-gas
                (+
                    {"balance" : new-balance}
                    (remove "balance" (UR_TrueFungible account snake-or-gas))
                )
            )
        )
    )
