...
(module ATSU GOV
    ...
    (defcap ATSU|C>CURL (ats1:string ats2:string curl-token:string)
        ...
        (let
            ...
            (enforce (or (not h1) (not h2)) "Cannot Curl when one of the ATS-Pairs has Hibernation turned on")
            (compose-capability (P|TT))
        )
    )
    ...
    (defun C_Coil:object{IgnisCollectorV2.OutputCumulator}
        ...
        (let
            (
                ...
                (c-rbt-amount:decimal (ref-ATS::URC_RBT ats rt input-amount))
            )
    ...
    (defun C_Curl:object{IgnisCollectorV2.OutputCumulator}
        ...
        @doc "Coils through 2 ATS-Pairs, outputting the <c-rbt2> to the <curler> \
            \ <ats1> must have <hibernation> off, and <ats2> may have it either way \
            \ If <ats2> has <hibernation> on, <c-rbt2-amount> is retained \
            \ and it will be hibernated from the TALOS module to the <curler>"
        (UEV_IMC)
        (let
            (
                (ref-U|ATS:module{UtilityAtsV2} U|ATS)
                ...
                (ref-DPTF:module{DemiourgosPactTrueFungibleV6} DPTF)
                ;;
                (royalty1:decimal (ref-ATS::UR_Royalty ats1))
                (rt-prec:integer (ref-DPTF::UR_Decimals rt))
                (royalty1-split:[decimal] (ref-U|ATS::UC_PromilleSplit royalty1 amount rt-prec))
                (input1-amount:decimal (at 0 royalty1-split))
                (royalty1-fee:decimal (at 1 royalty1-split))
                (c-rbt1:string (ref-ATS::UR_ColdRewardBearingToken ats1))
                (c-rbt1-amount:decimal (ref-ATS::URC_RBT ats1 rt input1-amount))
                ;;
                (royalty2:decimal (ref-ATS::UR_Royalty ats2))
                (c-rbt1-prec:integer (ref-DPTF::UR_Decimals c-rbt1))
                (royalty2-split:[decimal] (ref-U|ATS::UC_PromilleSplit royalty2 c-rbt1-amount c-rbt1-prec))
                (input2-amount:decimal (at 0 royalty2-split))
                (royalty2-fee:decimal (at 1 royalty2-split))
                (c-rbt2:string (ref-ATS::UR_ColdRewardBearingToken ats2))
                (c-rbt2-amount:decimal (ref-ATS::URC_RBT ats2 c-rbt1 input2-amount))
            )
            (with-capability (ATSU|C>CURL ats1 ats2 rt)
                (let
                    ...
                    (if (!= royalty2-fee 0.0)
                        (ref-ATS::XE_UpdateRUR ats2 rt 3 true royalty2-fee)
                        true
                    )
    ...
    (defun C_Recover:object{IgnisCollectorV2.OutputCumulator}
        ...
        @doc "Recovers a Hot-RBT Nonce, converting it to Cold-RBT in its entirety \
