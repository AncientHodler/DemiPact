...
(module ATS GOV
    ...
    (defcap ATS|C>TOGGLE-PARAMETER-LOCK (atspair:string toggle:bool)
        ...
        (let
            ...
            (if toggle
                ...
                (enforce (fold (and) true [(not cr) (not hr) (not dr)]) "ATSPair {} Recoveries must be stopped for exec" [atspair])
            )
            (compose-capability (SECURE))
        )
    )
    ...
    (defcap ATS|C>ADD-REWARD-TOKEN (atspair:string reward-token:string)
        ...
        (let
            (
                ...
                (n:integer (length UR_RewardTokens))
            )
    ...
    (defun UR_CanUpgrade:bool (atspair:string)
        (let 
            (
                ...
                (sentinel:string "__MISSING_<can-upgrade>__")
                (temp (try sentinel (at "can-upgrade" (read ATS|Pairs atspair ["can-upgrade"]))))
                (needs-populate:bool (= (typeof temp) "string"))
                (link:bool (if needs-populate default-value temp))
            )
            (if needs-populate
    ...
    (defun UR_Hibernate:bool (atspair:string)
        (let 
            (
                ...
                (sentinel:string "__MISSING_<hibernate>__")
                (temp (try sentinel (at "hibernate" (read ATS|Pairs atspair ["hibernate"]))))
                (needs-populate:bool (= (typeof temp) "string"))
                (link:bool (if needs-populate default-value temp))
            )
            (if needs-populate
    ...
    (defun UR_Royalty:decimal (atspair:string)
        (let 
            (
                ...
                (sentinel:string "__MISSING_<royalty-promile>__")
                (temp (try sentinel (at "royalty-promile" (read ATS|Pairs atspair ["royalty-promile"]))))
                (needs-populate:bool (= (typeof temp) "string"))
                (link:decimal (if needs-populate default-value temp))
            )
            (if needs-populate
                (update ATS|Pairs atspair
    ...
    (defun UR_PeakHibernatePromile:decimal (atspair:string)
        (let 
            (
                ...
                (sentinel:string "__MISSING_<peak-hibernate-promile>__")
                (temp (try sentinel (at "peak-hibernate-promile" (read ATS|Pairs atspair ["peak-hibernate-promile"]))))
                (needs-populate:bool (= (typeof temp) "string"))
                (link:decimal (if needs-populate default-value temp))
            )
            (if needs-populate
    ...
    (defun UR_HibernateDecay:decimal (atspair:string)
        (let 
            (
                ...
                (sentinel:string "__MISSING_<hibernate-decay>__")
                (temp (try sentinel (at "hibernate-decay" (read ATS|Pairs atspair ["hibernate-decay"]))))
                (needs-populate:bool (= (typeof temp) "string"))
                (link:decimal (if needs-populate default-value temp))
            )
            (if needs-populate
    ...
    (defun UR_DirectRecoveryFee:decimal (atspair:string)
        (let 
            (
                (default-value:decimal 0.0)
                (sentinel:string "__MISSING_<d-promile>__")
                (temp (try sentinel (at "d-promile" (read ATS|Pairs atspair ["d-promile"]))))
                (needs-populate:bool (= (typeof temp) "string"))
                (link:decimal (if needs-populate default-value temp))
            )
            (if needs-populate
    ...
    (defun UR_ToggleDirectRecovery:bool (atspair:string)
        (let 
            (
                (default-value:bool false)
                (sentinel:string "__MISSING_<direct-recovery>__")
                (temp (try sentinel (at "direct-recovery" (read ATS|Pairs atspair ["direct-recovery"]))))
                (needs-populate:bool (= (typeof temp) "string"))
                (link:bool (if needs-populate default-value temp))
            )
            (if needs-populate
    ...
    (defun URC_RewardTokenPosition:integer (atspair:string reward-token:string)
        ...
        (let
            (
                (ref-U|LST:module{StringProcessor} U|LST)
                (reward-token-lst:[integer] (UR_RewardTokenList atspair))
    ...
    (defun C_Control:object{IgnisCollectorV2.OutputCumulator}
        ...
        (let
            (
                (ref-IGNIS:module{IgnisCollector} IGNIS)
            )
            (with-capability (ATS|S>CONTROL atspair hibernate)
                (XI_Control can-change-owner syphoning hibernate)
    ...
    (defun C_UpdateSyphon:object{IgnisCollectorV2.OutputCumulator}
        ...
        (let
            ...
            (with-capability (ATS|S>ROYALTY atspair syphon)
    ...
    (defun C_AddSecondary:object{IgnisCollectorV2.OutputCumulator}
        ...
        (let
            ...
            (with-capability (ATS|C>ADD-REWARD-TOKEN atspair reward-token)
                (ref-DPTF::XB_DeployAccountWNE reward-token ATS|SC_NAME)
    ...
    (defun C_AddHotRBT:object{IgnisCollectorV2.OutputCumulator}
        ...
        (let
            ...
            (with-capability (ATS|C>ADD-HOT-RBT atspair hot-rbt)
                (ref-DPOF::XB_DeployAccountWNE hot-rbt ATS|SC_NAME)
    ...
    (defun XE_Update_RUR (atspair:string reward-token:string rur:integer direction:bool amount:decimal)
        ...
        (let
            (
                ...
                (royalty:decimal (at rtp (UR_RewardTokenRUR atspair 1)))
                ;;
                (rur-amount:decimal
                    (cond
