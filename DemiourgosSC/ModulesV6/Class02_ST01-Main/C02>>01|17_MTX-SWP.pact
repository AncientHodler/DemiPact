...
(module MTX-SWP GOV
    ...
    ;;
    (defcap MTX-SWP|C>ADD-STANDARD-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        @event
        (compose-capability (MTX-SWP|C>X-ADD-LQ swpair ld))
    )
    (defcap MTX-SWP|C>ADD-ICED-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        @event
        (compose-capability (MTX-SWP|C-ADD-CHILLED-LQ swpair ld))
    )
    (defcap MTX-SWP|C>ADD-GLACIAL-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        @event
        (compose-capability (MTX-SWP|C-ADD-CHILLED-LQ swpair ld))
    )
    (defcap MTX-SWP|C>ADD-FROZEN-LQ 
        (swpair:string frozen-dptf:string ld:object{SwapperLiquidity.LiquidityData})
        @event
        (let
            (
    ...
    (defcap MTX-SWP|C>ADD-SLEEPING-LQ 
        ...
        (let
            ...
            (ref-SWPLC::UEV_AddSleepingLiquidity account swpair sleeping-dpmf nonce)
            (compose-capability (MTX-SWP|C-ADD-DORMANT-LQ swpair ld))
        )
    )
    (defcap MTX-SWP|C-ADD-DORMANT-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        (let
            (
                (ref-SWPLC:module{SwapperLiquidityClient} SWPLC)
            )
    ...
    (defcap MTX-SWP|C-ADD-CHILLED-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        (let
            (
                (ref-SWPLC:module{SwapperLiquidityClient} SWPLC) 
            )
    ...
    (defcap MTX-SWP|C>X-ADD-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        (let
            (
                (ref-SWPLC:module{SwapperLiquidityClient} SWPLC) 
            )
    ...
    (defun C_AddSleepingLiquidity
        ...
        (UEV_IMC)
        (with-capability (MTX-SWP|S>ADD-LQ kda-pid)
            (MTX|C_AddSleepingLiquidity patron account swpair sleeping-dpmf nonce kda-pid)
        )
    )
    ;;{F6.P}  [MTX|C]
        ...
        (step
            (let
                ...
                (ref-IGNIS::IC|C_Collect patron 
                    (ref-IGNIS::IC|UDC_ConstructOutputCumulator 100.0 SWP|SC_NAME false [])
                )
        ...
        (step-with-rollback
            (resume
                ...
                (let
                    ...
                    (with-capability (MTX-SWP|C>ADD-SLEEPING-LQ account swpair sleeping-dpmf nonce ld)
                        (let
                            (
                                (ref-DALOS:module{OuronetDalosV4} DALOS)
                                (ref-DPMF:module{DemiourgosPactMetaFungibleV5} DPMF)
