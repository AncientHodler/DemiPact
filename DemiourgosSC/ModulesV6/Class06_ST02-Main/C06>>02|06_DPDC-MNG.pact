(module DPDC-MNG GOV
    ...
    (defun C_WipeSlim:object{IgnisCollector.OutputCumulator}
        ...
        (let
            ...
            (with-capability (DPDC-MNG|C>WIPE-SFT-NONCE-PARTIALLY account id nonce amount)
                ...
                (ref-IGNIS::IC|UDC_SmallestCumulator (ref-DPDC::UR_OwnerKonto id true))
            )
        )
    )
    ;;  [NFT]
    ...
    ;;  [SFT+NFT]
    (defun C_WipeNonce:object{IgnisCollector.OutputCumulator}
            ...
            (if son
                (let
                    ...
                    (with-capability (DPDC-MNG|C>WIPE-SFT-NONCE-TOTALLY account id nonce amount)
                        (XI_DecreaseClassZeroSemiFungibles account id [nonce] [amount] true)
                        (ref-IGNIS::IC|UDC_SmallCumulator owner)
                    )
                )
    ...
    ;;  [DEBIT-MULTIPLE]
    ;;  [SFT+NFT]
    (defun C_WipeHeavy:object{IgnisCollector.OutputCumulator}
    ...
    (defun C_WipeDirty:object{IgnisCollector.OutputCumulator}
        (account:string id:string son:bool nonces:[integer])
