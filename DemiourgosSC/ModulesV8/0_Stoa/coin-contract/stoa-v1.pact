...
(module STOA GOV
    ...
    (defschema FoundationPendingSchema
        fpa:decimal                     ;;Foundation Pending Amount, for (!= chain-id 0)
        last-fpa:decimal                ;;Last FPA, for (= chain-id 0)
    )
    (defschema FoundationSplit
        miner:decimal
        foundation:decimal
    )
    ;;{2}
    ...
    (defun UC_FoundationSplit:object{FoundationSplit} (amount:decimal)
        (let
            (
                (ten:decimal (floor (* amount 0.1) STOA_PREC))
                (ninety:decimal (- amount ten))
            )
            {"miner"        : ninety
            ,"foundation"   : ten}
        )
    ...
    (defun UR_FPA:decimal ()
        (with-default-read FoundationPending FPA
            {"fpa": 0.0}
            {"fpa" := supply}
            supply
        )
    )
    (defun UR_LastFPA:decimal ()
        (with-default-read FoundationPending FPA
            {"last-fpa": 0.0}
            {"last-fpa" := supply}
            supply
    ...
    (defun X_IncrementFPASupply (amount:decimal)
        (require-capability (UPDATE-FPA))
        ;(UEV_StoaPrecision amount) - Split takes care of precision
        (let
            (
                (current-fpa:decimal (UR_FPA))
                (incremented-fpa:decimal (+ current-fpa amount))
            )
            (write FoundationPending FPA
                {"fpa" : incremented-fpa}
            )
        )
    )
    (defun X_UpdateLastFPA (new-last-fpa:decimal)
        (require-capability (UPDATE-FPA))
        ;(UEV_StoaPrecision amount) - Split takes care of precision
        (write FoundationPending FPA
            {"last-fpa" : new-last-fpa}
        )
    )
    ;;
    (defun XM_StoaCoinbase:string (account:string account-guard:guard)
        ...
        (let
            (
                ...
                (chains:decimal (dec (length VALID_CHAIN_IDS)))
                (divisor:decimal (fold (*) 1.0 [BPD EMISSION-SPEED chains]))
                (yang-amount:decimal (floor (/ remaining divisor) STOA_PREC))
                ;;
                (fs:object{FoundationSplit} (UC_FoundationSplit yang-amount))
                (miner-share:decimal (at "miner" fs))
                (foundation-share:decimal (at "foundation" fs))
                (chain-id:string (at "chain-id" (chain-data)))
            )
            ;;
            (if (= chain-id "0")
                ;; CHAIN 0: Mint foundation share + accumulated from other chains + 90% of block to Miner
                ;; Miner keeps its 90% of Block, and injects the rest to URSTOA-Vault
                (let
                    (
                    ...
                    ;;Miner gets its 90% plus <total-foundation>
...
(create-table FoundationPending)
