(module TALOS GOVERNANCE
    ...
    ;;  MODULE  || SWAPPER
    ;;
    ;; 01] TALOS.SWP|C_ChangeOwnership                  ||  SWPM.SWPM|C_ChangeOwnership                             ||  patron:string atspair:string new-owner:string
    ;; 02] TALOS.SWP|C_ModifyCanChangeOwner             ||  SWPM.SWPM|C_ModifyCanChangeOwner                        ||  patron:string atspair:string new-boolean:bool
    ;; 03] TALOS.SWP|C_ToggleFeeLock
    (defun SWP|C_ToggleFeeLock (patron:string swpair:string toggle:bool)
        @doc "Sets the <fee-lock> for the SWP Pair <swpair> to <toggle> \
            \ Unlocking <fee-toggle> (setting it to false) comes with specific restrictions: \
            \       As many unlocks can be executed for a SWP Pair as needed \
            \       The Cost for unlock is (1000 IGNIS + 10 KDA )*(1 + <unlocks>)"
        (with-capability (S)
