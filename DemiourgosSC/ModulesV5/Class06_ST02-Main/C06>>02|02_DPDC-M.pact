(module DPDC-M GOV
    ...
    ;;GOVERNANCE
    ;;{G1}
    (defconst GOV|MD_DPDC-M        (keyset-ref-guard (GOV|Demiurgoi)))
    ;;{G2}
    (defcap GOV ()                  (compose-capability (GOV|DPDC-M_ADMIN)))
    (defcap GOV|DPDC-M_ADMIN ()    (enforce-guard GOV|MD_DPDC-M))
    ;;{G3}
    ...
    ;;{P3}
    (defcap P|DPDC-M|CALLER ()
        true
    )
    (defcap P|SECURE-CALLER ()
        (compose-capability (P|DPDC-M|CALLER))
    ...
    (defun P|A_Add (policy-name:string policy-guard:guard)
        (with-capability (GOV|DPDC-M_ADMIN)
    ...
    (defun P|A_Define ()
        (let
            (
                ...
                (mg:guard (create-capability-guard (P|DPDC-M|CALLER)))
            )
    ...
    (defun C_TogglePause:object{IgnisCollector.OutputCumulator}
        ...
        (let
            ...
            (with-capability (DPDC|S>TG_PAUSE id sft-or-nft toggle)
                ...
                (ref-IGNIS::IC|UDC_MediumCumulator (ref-DPDC::UR_OwnerKonto id sft-or-nft))
            )
        )
    )
    ;;Role Toggling
    ...
    (defun C_ToggleFreezeAccount:object{IgnisCollector.OutputCumulator}
        ...
        (let
            ...
            (with-capability (DPDC|C>FRZ-ACC id sft-or-nft account toggle)
                ...
                (ref-IGNIS::IC|UDC_BiggestCumulator (ref-DPDC::UR_OwnerKonto id sft-or-nft))
            )
        )
    )
    ...
    (defun C_ToggleBurnRole:object{IgnisCollector.OutputCumulator}
        ...
        (let
            ...
            (with-capability (DPDC|C>TG_BURN-R id sft-or-nft account toggle)
                ...
                (ref-IGNIS::IC|UDC_BigCumulator (ref-DPDC::UR_OwnerKonto id sft-or-nft))
            )
        )
    )
    ...
    (defun XI_IssueDigitalCollection:string
        ...
        (let
            (
                ...
                (specifications:object{Dpdc.DPDC|PropertiesSchema}
                ...
                (zse:[object{Dpdc.DPDC|Set}]
                    [(ref-DPDC::UDC_ZeroSet)]
                )
                (zne:[object{Dpdc.DPDC|NonceElementSchema}]
    ...
    (defun XI_TogglePause (id:string sft-or-nft:bool toggle:bool)
        ...
        (let
            (
                (ref-DPDC:module{Dpdc} DPDC)
                (new-specs:object{Dpdc.DPDC|PropertiesSchema}
                    (+
                        {"is-paused" : toggle}
    ...
    (defun XI_ToggleAddQuantityRole (id:string account:string toggle:bool)
        ...
        (let
            (
                ...
                (prp-roles:object{Dpdc.DPDC|RolesStorageSchema}
                    (+
    ...
    (defun XI_ToggleFreezeAccount (id:string sft-or-nft:bool account:string toggle:bool)
        ...
        (let
            (
                ...
                ;;
                (acc-roles:object{Dpdc.DPDC|RolesSchema}
                    (+
                        {"frozen"   : toggle}
