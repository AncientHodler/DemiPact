(module SWPLC GOV
    ...
    (implements SwapperLiquidityClient)
    ;;
    ;;<========>
    ;;GOVERNANCE
    ;;{G1}
    ...
    ;;
    (defcap SWPLC|C>ADD-STANDARD-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        @event
        (compose-capability (SWPLC|C>X-ADD-LQ swpair ld))
    )
    (defcap SWPLC|C>ADD-ICED-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        @event
        (compose-capability (SWPLC|C-ADD-CHILLED-LQ swpair ld))
    )
    (defcap SWPLC|C>ADD-GLACIAL-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
        @event
        (compose-capability (SWPLC|C-ADD-CHILLED-LQ swpair ld))
    )
    ...
    (defcap SWPLC|C-ADD-CHILLED-LQ (swpair:string ld:object{SwapperLiquidity.LiquidityData})
    ...
    (defun C|KDA-PID_AddSleepingLiquidity:object{IgnisCollector.OutputCumulator}
        ...
        (let
            ...
            (with-capability (SWPLC|C>ADD-SLEEPING-LQ account swpair sleeping-dpmf nonce ld)
                (let
                    (
                        (ref-IGNIS:module{IgnisCollector} DALOS)
                        (ref-DALOS:module{OuronetDalosV4} DALOS)
